...
def mmlabNormalize_RGB(img):
    # from mmcv.image.photometric import imnormalize
    # mean = np.array([123.675, 116.28, 103.53], dtype=np.float32)
    # std = np.array([58.395, 57.12, 57.375], dtype=np.float32)
    # to_rgb = True
    # img = imnormalize(np.array(img), mean, std, to_rgb)
    img = torch.tensor(np.array(img)).float().permute(2, 0, 1).contiguous()
    return img
...
class PrepareImageInputs(object):
    ...
    def __init__(
        ...
        sequential=False,
        ego_cam='CAM_FRONT',
        normalize_img='unchange',
    ):
        self.is_train = is_train
        self.data_config = data_config
        self.normalize_img = normalize_img
        if normalize_img == 'mod':
            self.normalize_img = mmlabNormalize_RGB
        elif normalize_img == 'unchange':
    ...
    def sample_augmentation(self, H, W, flip=None, scale=None):
        ...
        if self.is_train:
            ...
            # print(666, crop)  # (0, 140, 704, 396)
            # print(777, resize) # 0.44
...
@PIPELINES.register_module()
class PrepareImageInputs_advtrain(object):
    """Load multi channel images from a list of separate channel files.
    ...
    def __init__(
        ...
        # self.advimg = []
        # self.advimg.append('atk_exp/advtrain/img_00_000000_0.jpg')

        self.advimg = 'atk_exp/advtrain/img_00_%06d_0.jpg'

        self.img_nums = 300   # img num 300*8*6
        # self.img_nums = len(self.advimg)
    ...
    def normalize_img(self, img):
        ...
        if np.random.randint(0, 10) < 3:  # 30%
            curr_ins = np.random.randint(0, self.img_nums)
            img_name = self.advimg % curr_ins
            mask_name = img_name.replace('img', 'mask')

            curr_advimg = Image.open(img_name)
            curr_advimg = np.array(curr_advimg)

            curr_advmask = Image.open(mask_name)
            curr_advmask = np.array(curr_advmask)

            height, width = 396, 704
            curr_advimg = cv2.resize(curr_advimg, (width, height))
            curr_advimg = curr_advimg[140:396, :704, :]

            curr_advmask = cv2.resize(curr_advmask, (width, height))
            curr_advmask = curr_advmask[140:396, :704, :] / 255

            # cv2.imwrite('work_dirs/adv_train/tmp.jpg', img[..., [2,1,0]])
            img = img * (1 - curr_advmask) + curr_advimg * curr_advmask
            # cv2.imwrite('work_dirs/adv_train/tmp_1.jpg', img[..., [2,1,0]])
