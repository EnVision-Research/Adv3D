...
class PatchGen(nn.Module):
    def __init__(self, args, sem_index):
        super().__init__()
        self.args = args
        self.lc_list = []
        ...
        # self.generator.sem_index = sem_index
        self.sample_points_num = 48
...
def main():
    ...
    # semantic = [1, 3, 6, 8, 21, 23, 28, 29]

    # for sem_index in range(32):
    # if 1:
    with torch.no_grad():
        sem_index = 8
        ...
        patch_net = PatchGen(args, sem_index)
        ...
        # patch_net.module.lc_list[0] = torch.load('semantic_exp/lc_folder/lc_08_0000.pth')


        optimizer = optim.Adam(patch_net.module.lc_list[0]['lc_color'], lr=1e-3)

        depth_map = np.load('/perception/lilelife/data/nus/sample_depth_num/CAM_FRONT/n015-2018-08-03-15-00-36+0800__CAM_FRONT__1533279864662460.npy')

        curr_depth = torch.from_numpy(depth_map[1]).to(device)
        dense_depth = torch.from_numpy(depth_map[0]).to(device)


        from torchvision.io import read_image
        raw_bckg = read_image('paper_fig/raw_bckg.jpg')


        for curr_epoch in range(6):
            ...
            for i, data in enumerate(tqdm.tqdm(data_loader_train)):
                ...
                for curr_batch in range(len(data['gt_labels_3d'].data[0])):
                    ...
                    cam_para = data['img_inputs'][3][curr_batch][:1]
                    ...
                    img_edit = raw_bckg.unsqueeze(0).to(device)
                    curr_img = raw_bckg.unsqueeze(0).to(device)
                    adv_patch, adv_mask, adv_label = patch_add[curr_view][curr_advobj]
                    adv_patch = adv_patch[:, [2,1,0], ...]

                    # curr_img = curr_img * (1 - adv_mask) + adv_patch * adv_mask
                    # utils.save_image(curr_img / 255, f'paper_fig/fig1.jpg')

                    # adv_patch[adv_mask[:,[0,0,0],...] == 0] = 255
                    # utils.save_image(adv_patch / 255, f'paper_fig/foreground.jpg')



                    for depth_insert in np.linspace(10, 20, 21):
                        curr_mask = adv_mask.clone()
                        curr_edit = curr_img.clone()

                        curr_mask[:, :, dense_depth < depth_insert] = 0
                        curr_edit = curr_edit * (1 - curr_mask) + adv_patch * curr_mask
                        utils.save_image(curr_edit / 255, f'paper_fig/fig1_{depth_insert}.jpg')




                    object_depth = [adv_label[2][0][2]]

                    dep_h, dep_w = curr_depth.shape
                    bkg_img = img_edit[curr_batch:curr_batch+1, :, :dep_h, :dep_w].clone()
                    bkg_img[:, :, curr_depth!=0] = 0
                    dep_layer_list = list(np.arange(1, 11))[::-1]


                    min_depth, max_depth, bin_num = 0.1, 80, 10
                    maxmin_length = max_depth - min_depth

                    for add_layer in range(len(object_depth)):
                        add_depth = object_depth[add_layer]
                        for curr_layer in range(len(dep_layer_list)):
                            curr_bin = dep_layer_list[curr_layer] - 1
                            curr_min = min_depth + maxmin_length / bin_num * curr_bin
                            curr_max = min_depth + maxmin_length / bin_num * (curr_bin + 1)
                            if add_depth > curr_min and add_depth < curr_max:
                                dep_layer_list.insert(curr_layer + 1, -add_layer)


                    for dep_layer in dep_layer_list:
                        if dep_layer <= 0:
                            # bkg_img[:, :, curr_mask!=0] = curr_patch[curr_batch:curr_batch+1, :, :dep_h, :dep_w][:, :, curr_depth==dep_layer]
                            bkg_img = bkg_img * (1 - adv_mask) + adv_patch * adv_mask

                        else:
                            # import ipdb; ipdb.set_trace()
                            try:
                                bkg_img[:, :, curr_depth==dep_layer] = img_edit[curr_batch:curr_batch+1, :, :dep_h, :dep_w][:, :, curr_depth==dep_layer].to(bkg_img)
                            except:
                                print(dep_layer)
                                import ipdb; ipdb.set_trace()
                    ...
                    img_edit[curr_batch:curr_batch+1, :, :dep_h, :dep_w] = bkg_img
                    import ipdb; ipdb.set_trace()

                    utils.save_image(img_edit / 255, f'paper_fig/fig1_occ.jpg')
                ...
                if i == 0:
                    utils.save_image(img_edit.reshape((-1, 3, 256, 704))[:, [2,1,0], ...] / 255, f'semantic_exp/vis_folder/trains_%02d_%04d.jpg' % (sem_index, curr_epoch))
            ...
            # torch.save(patch_net.module.lc_list[0], 'semantic_exp/lc_folder/lc_%02d_%04d.pth' % (sem_index, curr_epoch))
            ...
            for i, data in enumerate(data_loader):
                with torch.no_grad():
                    ...
                    if i == 0:
                        utils.save_image(img_edit.reshape((-1, 3, 256, 704))[:, [2,1,0], ...] / 255, f'semantic_exp/vis_folder/vals_%02d_%04d.jpg' % (sem_index, curr_epoch))
