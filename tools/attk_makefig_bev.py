...
save_loca = 'paper_fig/fig_bev_set/train_atk/'
...
sys.path.append('/home/abc/adv3d/lift3d')
...
def lidar2img(points_lidar, camrera_info):
    ...
    valid = np.logical_and(points_camera[:, -1] > -4, valid)

    points_camera[:, 2] = np.abs(points_camera[:, 2])
    # points_camera[points_camera[:, 2] > -4][:, 2] = np.abs(points_camera[points_camera[:, 2] > -4][:, 2])
...
def lidar2img_raw(points_lidar, camrera_info):
    points_lidar_homogeneous = \
        np.concatenate([points_lidar,
                        np.ones((points_lidar.shape[0], 1),
                                dtype=points_lidar.dtype)], axis=1)
...
class PatchGen(nn.Module):
    def __init__(self, args, ):
        ...
        lc_loca = '/home/abc/code_recover/000235_1500.pth'
        self.lc_list.append(torch.load(lc_loca, map_location='cpu'))
        ...
        ckpt_path = '/home/abc/code_recover/models_0200000.pt'
    ...
    def forward(self, cam_info, ins_num=2):
        ...
        box_tensor[:, :, 2] = box_tensor[:, :, 2] * 5 + 12
...
def main():
    ...
    trans_lcs = []
    trans_lcs.append('atk_exp/lc_folder/lc_0004.pth')
    # trans_lcs.append('/home/abc/adv3d/BEVDet/semantic_exp/lc_folder/lc_08_0004.pth')
    ...
    # data_train['gt_labels_3d'].data[0][0] = data_train['gt_labels_3d'].data[0][0][13:14]
    # data_train['gt_bboxes_3d'].data[0][0].tensor = data_train['gt_bboxes_3d'].data[0][0].tensor[13:14]
    # data_train['gt_bboxes_3d'].data[0][0].tensor[:, 0] -= 4


    # data['gt_labels_3d'].data[0][0] = \
    #             torch.zeros_like(data['gt_labels_3d'].data[0][0]) + 5

      
    for i, data_test in enumerate(data_loader):
        curr_epoch = 0
        img_id = i
    ...
    # for curr_epoch in tqdm.tqdm(range(1000+1)):
        # data = copy.deepcopy(data_train)
        # optimizer.zero_grad()

        # img_normed = data_train['img_inputs'][0]
        # # back norm
        # # import ipdb; ipdb.set_trace()
        ...
        #     # array([[-3.21086592, 10.49352969, -0.79330671,  4.32      ,  1.81      ,
        #     #          1.509     ,  1.58875049]])
        ...
        # for img_id, data in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                result = model(return_loss=False, rescale=True, **data_test)
                print('here is the detected score before attack')
                print(result[0]['pts_bbox']['scores_3d'].sort(descending=True)[0][:10])
                raw_results = []
                ...
                raw_mtv_imgs, raw_canvas = make_img(raw_results, args, dataset, dataset_pkl, img_id)

                # data = copy.deepcopy(data_test)
                img_normed = data_test['img_inputs'][0][0]
                ...
                for curr_batch in range(data_test['img_inputs'][0][0].shape[0]):

                    cam_para = data_test['img_inputs'][0][3][curr_batch]#[1:2]
                    img_size = [(900, 1600, 3)] * cam_para.shape[0]
                    cam_info = [cam_para, img_size]
                    ...
                    for curr_view in range(img_edit.shape[1]):
                        ...
                        if 1:
                            for curr_advobj in range(len(patch_add[0])):
                                ...
                                tmp_list_fullsize.append([adv_patch, adv_mask])
                                # import ipdb; ipdb.set_trace()
                                # utils.save_image(adv_patch / 255, f'paper_fig/tmp/vals_{curr_view}.jpg')
                ...
                data_test['img_inputs'][0][0] = img_input

                result = model(return_loss=False, rescale=True, **data_test)

                print('here is the detected score after attack')
                print(result[0]['pts_bbox']['scores_3d'].sort(descending=True)[0][:10])
            ...
            for cnt, infos in enumerate(dataset_pkl['infos']):
                ...
                if 1:
                    # import ipdb; ipdb.set_trace()
                    tmp_info = infos['gt_boxes']
                    # tmp_info = infos['gt_boxes'][14:15, :] # select for target attack
                    
                    gt_boxes = copy.deepcopy(tmp_info)
                    ...
                    #     LB(infos['gt_boxes'],
                    #     origin=(0.5, 0.5, 0.5)).corners.numpy().reshape(-1, 3)
                    # corners_lidar_gt = np.concatenate([corners_lidar_gt[:, 2:3], corners_lidar_gt[:, :2]], -1)
                    ...
                    scores = scores + [1 for _ in range(tmp_info.shape[0])]
                ...
                for view_num, view in enumerate(views):
                    ...
                    for adv_num in range(val_ins_num):
                        if view_num in sel_view:
                            adv_patch, adv_mask = tmp_list_fullsize[view_num + adv_num]
                            adv_patch = adv_patch[0].permute(1,2,0).cpu().numpy()
                            adv_mask = adv_mask[0].permute(1,2,0).cpu().numpy()
                            img = img * (1 - adv_mask) + adv_patch * adv_mask
                ...
                cat_imgs = [*imgs[:], *raw_mtv_imgs[:]]
                # cat_imgs = [*imgs[:3], *raw_mtv_imgs[:3]]


                padding_size = 8
                fov_img = torch.cat([torch.from_numpy(x).permute(2,0,1)[None,...] for x in cat_imgs[:]], 0)
                fov_img = torchvision.utils.make_grid(fov_img, nrow=3, padding=padding_size, pad_value=255)
                fov_img = fov_img.permute(1,2,0).cpu().numpy()

                canvas_remap = cv2.resize(canvas, (fov_img.shape[0], fov_img.shape[0]))
                raw_canvas = cv2.resize(raw_canvas, (fov_img.shape[0], fov_img.shape[0]))
                

                # canvas_remap = np.concatenate([
                #     np.ones([padding_size, raw_canvas.shape[1], 3]) + 255, 
                #     canvas_remap[(padding_size*3)//2 + raw_canvas.shape[0]//2 : raw_canvas.shape[0], ...], 
                #     np.ones([padding_size, raw_canvas.shape[1], 3]) + 255, 
                #     raw_canvas[(padding_size*3)//2 + raw_canvas.shape[0]//2 : raw_canvas.shape[0], ...], 
                #     np.ones([padding_size, raw_canvas.shape[1], 3]) + 255
                #     ], axis=0)

                # canvas_remap = np.concatenate([
                #     np.ones([padding_size, raw_canvas.shape[1], 3]) + 255, 
                #     canvas_remap[(padding_size*3)//2:raw_canvas.shape[0]//2, ...], 
                #     np.ones([padding_size, raw_canvas.shape[1], 3]) + 255, 
                #     raw_canvas[(padding_size*3)//2:raw_canvas.shape[0]//2, ...], 
                #     np.ones([padding_size, raw_canvas.shape[1], 3]) + 255
                #     ], axis=0)

                final_img = np.concatenate([fov_img, canvas_remap], axis=1)
                cv2.imwrite(os.path.join(save_loca, '%06d_adv.jpg' % (img_id)), final_img)
            ...
            #         cv2.imwrite(os.path.join('paper_fig/fig_bev_remap/', '%06d_adv.jpg' % (img_id)), img)
...
def make_img(results, args, dataset, dataset_pkl, img_id):
    ...
    for cnt, infos in enumerate(dataset_pkl['infos']):
        ...
        fov_img = torch.cat([torch.from_numpy(x).permute(2,0,1)[None,...] for x in copy.deepcopy(imgs)], 0)
        fov_img = torchvision.utils.make_grid(fov_img, nrow=3, padding=8, pad_value=255)
        fov_img = fov_img.permute(1,2,0).cpu().numpy()

        canvas_remap = cv2.resize(canvas, (fov_img.shape[0], fov_img.shape[0]))
        final_img = np.concatenate([fov_img, canvas_remap], axis=1)
        cv2.imwrite(os.path.join(save_loca, '%06d_raw.jpg' % (img_id)), final_img)
        # cv2.imwrite(os.path.join('paper_fig/fig_bev_remap/', '%06d_raw.jpg' % (img_id)), final_img)

        return imgs, canvas_remap
