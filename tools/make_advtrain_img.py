...
class PatchGen(nn.Module):
    ...
    def forward(self, cam_info, ins_num=2):
        ...
        box_tensor[:, :, 2] = box_tensor[:, :, 2] * 5 + 15
...
def main():
    ...
    patch_net.module.lc_list[0] = torch.load('atk_exp_nocbgs/lc_folder/lc_0001.pth')
            ...
            for curr_batch in range(len(data['gt_labels_3d'].data[0])):
                ...
                for curr_view in range(len(patch_add)):
                    ...
                    for curr_advobj in range(len(patch_add[curr_view])):
                        ...
                        # utils.save_image(adv_patch / 255, 'atk_exp/advtrain/img_%02d_%06d_%d.jpg' % (curr_view, ins_num, rank))
                        utils.save_image(adv_patch[:, [2,1,0], ...] / 255, 'atk_exp/advtrain/img_%02d_%06d_%d.jpg' % (curr_view, ins_num, rank))
                        utils.save_image(adv_mask, 'atk_exp/advtrain/mask_%02d_%06d_%d.jpg' % (curr_view, ins_num, rank))
